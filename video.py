...
class Video:
    ...
    def check_extracted_pts(self):
        pts_file = osp.join(self.path, "frames.txt")
    ...
    def downscale_frames(
        self, subdir, max_size, ext, align=32, full_subdir="color_full",
        short_side_target=False,
    ):
        full_dir = osp.join(self.path, full_subdir)
        down_dir = osp.join(self.path, subdir)
